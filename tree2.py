...
class AVLTree2:
    ...
    def searchTree(self, k):
        search_result = self.__searchTreeHelper(self.root, k)
    ...
    def maximum(self, node):
        while node.right != None:
            node = node.right
        return node.value
    def findlargestleft(self, num, node):
        if node == None:
            return None
        elif node.data < num:
            tmp = self.findlargestleft(num,node.right)
            if tmp != None:
                return tmp
            else:
                return node.value
        else:
            tmp = self.findlargestleft(num,node.left)